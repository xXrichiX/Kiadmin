...
useEffect(() => {
  ...
  const calculateOrderStats = (orders) => {
    const stats = {
      ...
      inProgress: orders.filter(order => order.status === "en preparacion").length,
  ...
  const getProductSummary = (products) => {
    ...
    if (products.length > 2) {
      const firstTwo = products.slice(0, 2).map(p => 
        `${productMap[p.productId] || 'Producto'} (${p.quantity})`
      ).join(', ');
      
      return `${firstTwo} y ${products.length - 2} más`;
    }
  ...
  const DetailedProducts = ({ products, orderCurrency }) => {
    return (
        ...
        <table className="product-details-table">
          ...
          <tbody>
            {products.map((product, index) => (
              <tr key={index}>
                <td>{productMap[product.productId] || `Producto (${product.productId})`}</td>
                <td>{product.quantity}</td>
                <td>{formatPrice(product.price || 0, orderCurrency)}</td>
                <td>{formatPrice((product.price || 0) * (product.quantity || 1), orderCurrency)}</td>
              </tr>
            ))}
          </tbody>
        </table>
